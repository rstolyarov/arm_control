...
def sampleUser(test=0, supertest=0):
  if test == 0:
    ...
    for i in range(SAMPLE_NUMBER):
      for j in range(6):
        emg_vectors[j][i] = readChannel(j);
      time.sleep(SAMPLE_PERIOD)
    new_reads, rawRMS = processSignal(emg_vectors, supertest)
    print "User sampled (WRIST,ELBOW,CLAW):",new_reads
    return new_reads, rawRMS
  else:
